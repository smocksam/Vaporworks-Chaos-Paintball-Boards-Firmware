...
void fire() 
{
	...
	if(bit_is_set(EYEPORT,Eye) || nScope==0 || nEyeBypassAuto)
	{
		errcount=0;
...
void checkEyes()
{
	if(nEyeBypassManual==0 && nEyeBypassAuto==1 && bit_is_clear(EYEPORT,Eye)) 
		nEyeBypassAuto=0; 
}
...
SIGNAL(SIG_OVERFLOW0) //was: void fireTimer() 
{
	...
	if(EWait==0)
	{
		switch(nBStage)
		{
			case 0:					// Stage 0 - waiting for bolt to open
				if(nEyeMode>2)
				{
					if(bit_is_set(EYEPORT,Eye) && (nECount>0))         // Wait for up to EYEBYPASSMS (100ms) (init in 'fire' section)
					{
						nECount--;
						nTestVar=0;
					}
			...
			case 1:
				if(bit_is_clear(EYEPORT,Eye) && nECount>0 && nTestCPS==0)
					nECount--;
				else
				{
					nBStage=2;
					if(nECount!=0)
					{
						if(nAEDSetting) nECount=(nAEDSetting-1)*2;
					}
					else
					{
			...
			case 2:
				if(nEyeMode>2)
				{
					if(nECount!=0)
					{
						if(bit_is_set(EYEPORT,Eye))                    // Finally, run after-eye delay
				...
				else if(bit_is_set(EYEPORT,Eye))
				{
					if(nECount--==0) 
						nBStage=3;
				}
	...
	else
	{
		...
		if(nEyeMode>2)
		{
			if(EWait==0 && (bit_is_clear(EYEPORT,Eye) && !nBStage))
			{
				if(++errcount<50)
					EWait = 1;
			}
		...
		{
			if(bit_is_clear(EYEPORT,Eye))
			{
				errcount=1; // When in bolt not visible, we'll use this for
				// As long as we see open breech sometime, this'll get set.
			}
			if(!errcount && !EWait)
			{
			// If errcount doesn't get set, and ewait runs out, bypass the eye
				nEyeBypassAuto=1;                    // Bypass if it doesn't open in time.
				nBStage = 3;
			}
		}
