...
VTPORT	EQU	0A0h		; 
VTBIT	EQU	4		; 
FT1	EQU	68D		; 20/UL bps
FT2	EQU	68D		; 20/UL bps
FT3	EQU	68D		; 20/UL bps
FT4	EQU	68D		; 20/UL bps
FT5	EQU	68D		; 20/UL bps
FT6	EQU	68D		; 20/UL bps
FT7	EQU	68D		; 20/UL bps
FT8	EQU	68D		; 20/UL bps
DFT1	EQU	68D		; 20/UL bps
DFT2	EQU	68D		; 20/UL bps
DFT3	EQU	68D		; 20/UL bps
DFT4	EQU	68D		; 20/UL bps
DFT5	EQU	68D		; 20/UL bps
DFT6	EQU	68D		; 20/UL bps
DFT7	EQU	68D		; 20/UL bps
DFT8	EQU	68D		; 20/UL bps
OS1	equ	'1'		;
OS2	equ	'1'		;
OS3	equ	'1'		;
OS4	equ	'1'		;
...
OTICK		equ	070h		;					;
n3sbyte		equ	071h		;
FULLAUTOTMRL	equ	072h
FULLAUTOTMRH	equ	073h
					;
STACK		EQU	074H		; Stack starts here, LOTSA room still left
					;
	.list on			;
	.program			;
RESET:					;
...
XTM10:                                  ; 
        ...
        call    WaitRelease             ; Wait for trigger release 
XTM20:     
		;DW-removing tourney lock check, this really isn't needed in this cheater board. :) 
...
XTM27FULL:
        ...
        MOV     TMRS,#000H              ; Clear power-down seconds counter 
        MOV     TMRM,#000H              ; Clear power-down minutes counters 
		jnb	TPORT.TBIT,XTM27FULL
        CALL    WaitRelease             ; Wait for trigger release         
		setb	IE.0					;
        sjmp	XTM20 					;

OFF_FULLAUTO:
	mov		a, n3sbyte		; check if it is in full auto mode
	add  	A, #0
	subb 	A, #3
	jc 	XTM20
	;cjne	a, #3, XTM20	; if not in full auto then go back to main loop
	mov		a, FULLAUTOTMRH	; 
	add  	A, #0
	subb 	A, #4
	jc 	XTM20
	;cjne	a, #4, XTM20	; if it is not yet 1 (500ms * 2) sec then go back to main loop
	mov		n3sbyte, #0d	; go to semi-auto mode
	sjmp	XTM20
...
TMR0:					;
	...
	MOV	TL0,#T0LSB		;
	inc	FULLAUTOTMRL	; jtan, increment FULLAUTOTIMER
	mov	A, FULLAUTOTMRL
;	subb A, #250d
;	jc UPDATETICK
	cjne	A, #250d, UPDATETICK
	mov	FULLAUTOTMRL, #0d	
	mov	A, FULLAUTOTMRH
	add  A, #0
	subb A, #4
	jnc	UPDATETICK
	inc	FULLAUTOTMRH	; means 500ms has elapsed
UPDATETICK:
