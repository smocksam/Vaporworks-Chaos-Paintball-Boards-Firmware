...
XTM20:                                  ; 
        ...
        jnb     lcdupdate,XTM22 	; check for lcd update 
        ACALL   UPDISP                  ; If so, run it 
XTM22:                                  ; 
        jnb     PDTIMER,XTM25           ; Stay powered up? 
        JMP     PowerDown               ; Shutdown 
XTM25:                                  ; 
        jb      B1PORT.B1BIT,XTM26      ; check buttons 
        acall   BPROCESS                ; process them 
XTM26:                                  ; 
        jb      B2PORT.B2BIT,XTM27      ; check buttons 
        acall   BPROCESS                ; process them 
XTM27:                                  ; 
        jb      RSCHG,NOTPROG2          ; check to see if mode has changed 
        mov     A,TRIGBIT               ; check to see if trigger bit is set 
        jz      XTM20                   ; if not, go back to beginning 
        mov     a,OTICK			; 
        jnz     XTM28B                  ; 
        mov     TRIGBIT,#0d             ; 
        SETB    wfire                   ; 
