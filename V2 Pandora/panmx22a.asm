...
OTICK		equ	070h		;					;
					;
STACK		EQU	071H		; Stack starts here, LOTSA room still left
					;
	.list on			;
	.program			;
RESET:					;
...
NOTPROG2:				;
	...
	mov	DPTR,#MN10		; point to jump table
MN05:					;
	jmp	@A+DPTR			; and jump out
MN10:					;
	ljmp	SAM10a			; Semi-auto mode selected (0)
	ljmp	TSAM10a			; Turbo mode selected (1)
	ljmp	ARM10a			; Auto-response mode selected (2)
	ljmp	FAM10a			; Full-auto mode is selected (3)
; ---------------------------------------------------------------------
;                           Semi-Auto Mode
; ---------------------------------------------------------------------
;					;
...
TSAM10a:				;
	...
	clr	firing			;
TSAM10:					;
	setb	LPORT.LBIT		; Turn on LED
	call	WaitRelease		; Wait for trigger release
TSAM20:					;
	acall	TLCHECK			; Check tourney lock
	jnb	lcdupdate,TSAM22	; check for lcd update
	ACALL	UPDISP			; If so, run it
TSAM22:					;
	jnb	PDTIMER,TSAM25		; Stay powered up?
	JMP	PowerDown		; Shutdown
TSAM25:					;
	jb	B1PORT.B1BIT,TSAM26	; check buttons
	acall	BPROCESS		; process them
TSAM26:					;
	jb	B2PORT.B2BIT,TSAM27	; check buttons
	acall	BPROCESS		; process them
TSAM27:					;
	jb	RSCHG,NOTPROG2		; check to see if mode has changed
	mov	A,TRIGBIT		; check to see if trigger bit is set
	jz	TSAM20			; if not, go back to beginning
	mov	TRIGBIT,#0d		;
	SETB	wfire			; 
	MOV	TMRS,#000H		; Clear power-down seconds counter
	MOV	TMRM,#000H		; Clear power-down minutes counters
	CALL	WaitRelease		; Wait for trigger release
TARM20:					;
	acall	TLCHECK			; Check tourney lock status 
	jnb	lcdupdate,TARM22	; check lcd update status
	ACALL	UPDISP			; 
TARM22:					; 
	jnb	PDTIMER,TARM25		; Stay powered up?
	JMP	PowerDown		; Shutdown
TARM25:					; 
	jb	B1PORT.B1BIT,TARM26	; 
	acall	BPROCESS		; 
TARM26:					; 
	jb	B2PORT.B2BIT,TARM27	; 
	acall	BPROCESS		; 
TARM27:					; 
	jb	RSCHG,MAINJMP		; 
	mov	A,TRIGBIT		; 
	jz	TARM20			; 
	mov	TRIGBIT,#0d		; 
	;JB	TPORT.TBIT,TARM20	; Is the trigger pressed?
	MOV	TICK,#250		; Load auto-response trigger timer
	SETB	wfire			;
	MOV	TMRS,#000H		; Clear power-down seconds counter
	MOV	TMRM,#000H		; Clear power-down minutes counters
TARM40:					;
	CALL	WaitRelease		; Wait for trigger release
	MOV	A,TICK			; Get the auto-response trigger timer
	JZ	TSAM20			; z=timed out, restart on next pull
	SETB	wfire			;
	MOV	TMRS,#000H		; Clear power-down seconds counter
	MOV	TMRM,#000H		; Clear power-down minutes counters
	CALL	WaitReturn		;
TARM60:					;
	SJMP	TSAM20			; and loop back for another round

					;
MAINJMP:				;
	jmp	NOTPROG2		; This is here as a 'stepping stone' for relative jumps
;					;
; ---------------------------------------------------------------------
;                           Auto-Response Mode
; ---------------------------------------------------------------------
...
ARM20:					;
	...
	ACALL	UPDISP			;
ARM22:					;
	jnb	PDTIMER,ARM25		; Stay powered up?
	JMP	PowerDown		; Shutdown
ARM25:					;
	jb	B1PORT.B1BIT,ARM26	;
	acall	BPROCESS		;
ARM26:					;
	jb	B2PORT.B2BIT,ARM27	;
	acall	BPROCESS		;
ARM27:					;
	jb	RSCHG,MAINJMP		;
	mov	A,TRIGBIT		;
	jz	ARM20			;
	mov	TRIGBIT,#0d		;
	;JB	TPORT.TBIT,ARM20	; Is the trigger pressed?
	MOV	TICK,#250		; Load auto-response trigger timer
	SETB	wfire			;
	MOV	TMRS,#000H		; Clear power-down seconds counter
	MOV	TMRM,#000H		; Clear power-down minutes counters
	CALL	WaitRelease		; Wait for trigger release
	MOV	A,TICK			; Get the auto-response trigger timer
	JZ	ARM20			; z=timed out, restart on next pull
	SETB	wfire			;
	CALL	WaitReturn		;
	SJMP	ARM20			; and loop back for another round
					;
;					;
; ---------------------------------------------------------------------
;                             Full-Auto Mode
...
FAM21:					;
	...
	ACALL	UPDISP			;
FAM22:					;
	jnb	PDTIMER,FAM25		; Stay powered up?
	JMP	PowerDown		; Shutdown
FAM25:					;
	jb	B1PORT.B1BIT,FAM26	;
	acall	BPROCESS		;
FAM26:					;
	jb	B2PORT.B2BIT,FAM27	;
	acall	BPROCESS		;
FAM27:					;
	jb	RSCHG,MAINJMP		;
	mov	A,TRIGBIT		;
	jz	FAM20			;
	mov	TRIGBIT,#0d		;
	;JB	TPORT.TBIT,FAM20	; Is the trigger pressed?
FAM28:					;
	SETB	wfire			;
	MOV	TMRS,#000H		; Clear power-down seconds counter
	MOV	TMRM,#000H		; Clear power-down minutes counters
	CALL	WaitReturn		;
	JNB	TPORT.TBIT,FAM28	; Is the trigger still pressed?
	setb	IE.0			;
	SJMP	FAM21			; and loop back
					;
;					;
; ======================================================================
